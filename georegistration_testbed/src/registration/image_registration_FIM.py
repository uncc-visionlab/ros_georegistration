...
class ImageRegistrationFIM(ImageRegistration):
    ...
    def calculateMutualInformation_fim(self, image_moving_bw, image_ref, transformAsVec):
        ...
        temp_array = np.multiply(warpedImage.copy(), temp_multiplier)
        image_ref_temp = np.multiply(temp_array, image_ref.copy())
        histogram2D_output = ImageRegistrationWithMutualInformation.histogramOutput(warpedImage, image_ref_temp, n_bins=self.numIntensityBins)
        mutualInformation = ImageRegistrationWithMutualInformation.mutualInformation(histogram2D_output)
        return mutualInformation

    def registerImagePair(self, image_moving, image_ref, GroundTruth, noisy_perturbation, AFFINE = True, OPT_NM = True, OPT_PSO = False):
        """
        ...
        minimizationErrorFunction_pso = lambda x: -self.calculateMutualInformation_pso(image_moving_bw, image_ref_scaled, x)

        if (self.OPT_DEFAULT == True):
            xopt_default = scipy.optimize.fmin(func=minimizationErrorFunction, x0 = noisy_transformAsVec, args = (), xtol = 0.1, ftol = 0.1, 
        ...
        if (self.OPT_NelderMeid == True):
            ...
            noisy_transformAsVec = np.array([max_a1, max_a2, max_b1, max_a3, max_a4, max_b2, max_c1, max_c2])
            xopt_nm = scipy.optimize.minimize(minimizationErrorFunction, x0=noisy_transformAsVec, args=(), method='Nelder-Mead', bounds=None, options={'disp': True})
        ...
        if (self.OPT_PSO == True):
            ...
            if (self.Affine_Xform == True):
                ...
                if (self.optimize_x):
                    ...
                    cost, xopt_pso_global_x = optimizer.optimize(objective_func=minimizationErrorFunction_pso, iters=pso_iter)
                    if (pso_plot_cost):
                ...
                if (self.xy_grid_search == True and pso_grid_search == False and pso_random_search == False):
                    ...
                    min_b1, max_b1, min_b2, max_b2 = int(min_b1.copy()), int(max_b1.copy()), int(min_b2.copy()), int(max_b2.copy())
                    xv, yv = np.meshgrid(np.linspace(min_b1, max_b1, self.xy_grid_points), np.linspace(min_b2, max_b2, self.xy_grid_points), indexing='ij')
                    ny, nx = len(yv), len(xv)
                ...
                if (self.optimize_6DOF == True):
                    if (self.xy_grid_search == True):
                        ...
                        optimizer = ps.single.GlobalBestPSO(n_particles=swarm, dimensions=dim, options=options_global, bounds=bounds_4DOF)
                        ...
                        cost, xopt_pso_global_4DOF = optimizer.optimize(objective_func=minimizationErrorFunction_pso, iters=pso_iter)
                        ...
                        optimizer = ps.single.GlobalBestPSO(n_particles=swarm, dimensions=dim, options=options_global, bounds=bounds_6DOF)
                        #options_local = {'w': 0.5069, 'c1': 2.5524, 'c2': 1.0056, 'k': 10, 'p': 2}
                        #optimizer = ps.single.LocalBestPSO(n_particles=swarm, dimensions=dim, options=options_local, bounds=bounds_6DOF)
                        cost, xopt_pso_global_6DOF = optimizer.optimize(objective_func=minimizationErrorFunction_pso, iters=pso_iter)
                        if (pso_plot_cost):
                            plot_cost_history(cost_history=optimizer.cost_history)
                            plt.show()
                ...
                if (self.optimize_total == True):
                    if (self.xy_grid_search == True):
                        ...
                        bounds = (min_bound, max_bound)
                        optimizer = ps.single.GlobalBestPSO(n_particles=swarm, dimensions=dim, options=options_global, bounds=bounds)
                        cost, xopt_pso_global = optimizer.optimize(objective_func=minimizationErrorFunction_pso, iters=pso_iter)
                ...
                if (pso_random_search == True and self.optimize_xy == True and self.optimize_6DOF == True):
                    ...
                    g = RandomSearch(ps.single.LocalBestPSO, n_particles= swarm, dimensions=dim, options=options_grid, objective_func= minimizationErrorFunction_pso, iters= pso_iter,
                    ...
                    options_updated = {'w': best_options['w'], 'c1': best_options['c1'], 'c2': best_options['c2'], 'k': best_options['k'], 'p': 1}
                    max_b = np.array([max_a1, max_a2, max_b1, max_a3, max_a4, max_b2])
                    min_b = np.array([min_a1, min_a2, min_b1, min_a3, min_a4, min_b2])
                    bounds_6DOF = (min_b, max_b)
                    dim_updated = len(min_b)
                    optimizer_updated = ps.single.LocalBestPSO(n_particles=swarm, dimensions=dim_updated, options=options_updated, bounds=bounds_6DOF)
                    cost, xopt_pso_global_6DOF = optimizer_updated.optimize(objective_func=minimizationErrorFunction_pso, iters=pso_iter)
                    xopt = self.GroundTruth
                    xopt[:6] = xopt_pso_global_6DOF[:6]
...
if __name__ == '__main__':
    ...
    sigmaHessian, f_Jacobian = ImageRegistrationFIMnode.fisherInformation(moving_dataset[idx], reference_dataset[idx], H_est[:8],
    ...
    #             plt.savefig('FI_Var_STD_%s.png' % str(kk), dpi=800)
    #         plt.show()
