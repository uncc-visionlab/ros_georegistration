...
class ROSImageRegistrationNode(object):
    ...
    def __init__(self):
        ...
        self.USE_REFERENCE_FOR_TRAJECTORY = True
        ...
        self.LOGGING = True
        self.IMAGE_DATASET = False
        self.IMAGE_DATASET_GT = False
        self.Affine_Xform = True
        self.PERSPECTIVE_Xform = True # self.PERSPECTIVE_Xform = False for AFFINE transform
        self.Without_MI_Pose = True # MI on = False, MI off = True
        self.reg_methods = ['withoutMI','affine','perspective']
        self.transformation_perturbation = 0.1
        self.ready = False
        ...
        if (self.LOGGING == True):
            ...
            self.datafile = open('/tmp/rosgeoregistration_' + datetime_str + '.txt', 'w')
        if (self.IMAGE_DATASET_GT == True):
            #datetime_str = str(datetime.now().strftime("%Y%m%d-%H%M%S"))
            datetime_str = str(time.datetime.now().strftime('%Y_%m_%d_%H_%M_%S'))
            self.datafile = open('/tmp/dataset_gt_' + datetime_str + '.txt', 'w')
    ...
    def sarCameraViewCallback(self, camera_view_msg):
        ...
        print("Sensed SAR image #: %d" % int(self.frame_id))
        try:
            ...
            if (self.register_moving_image is not None and 
                ...
                if (self.IMAGE_DATASET == True):
                    cv2.imwrite('/home/mhossai9/temp/dataset/fixed/registration_fixed_image_excerpt_'+str(self.frame_id)+'.png', self.register_fixed_image)
                    #print('Saving image '+'registration_moving_image_%s.png' % str(self.frame_id))
                    cv2.imwrite('/home/mhossai9/temp/dataset/moving/registration_moving_image_'+str(self.frame_id)+'.png', self.register_moving_image)
                ...
                if (self.ALGORITHM == Algorithm.OPENCV_FEATURES):
                    ...
                    if (self.Affine_Xform == True):
                        Hrgb_bbox_truth_affine = np.concatenate((Haffine_rgb_ref_truth, np.array([[0,0,1]])), axis=0)
                        initialTransform = np.linalg.inv(Hrgb_bbox_truth_affine)
                        initialTransform = initialTransform / initialTransform[2, 2]
                        transformAsVec = np.reshape(initialTransform, (1, 9))
                        transformAsVecAffine = transformAsVec[0, :6]
                        noiseVec = np.random.normal(0, 1, (1, 6))
                        noisy_transformAsVecAffine = transformAsVecAffine + self.transformation_perturbation*np.multiply(transformAsVecAffine,noiseVec)
                    
                    if (self.PERSPECTIVE_Xform == True):
                        Hrgb_bbox_truth_perspective = Hrgb_bbox_truth
                        initialTransform = np.linalg.inv(Hrgb_bbox_truth_perspective)
                        initialTransform = initialTransform / initialTransform[2, 2]
                        transformAsVec = np.reshape(initialTransform, (1, 9))
                        transformAsVecPerspective = transformAsVec[0, :8]
                        noiseVec = np.random.normal(0, 1, (1, 8))
                        noisy_transformAsVecPerspective = transformAsVecPerspective + self.transformation_perturbation*np.multiply(transformAsVecPerspective,noiseVec)
        
                    #if (self.Without_MI_Pose == True):
                     #   Hrgb_estimated_Affine_WithoutMI = noisy_transformAsVecAffine
                      #  Hrgb_estimated_Perspective_WithoutMI = noisy_transformAsVecPerspective
                    
                    (Hrgb_estimated_Affine, mask) = self.algorithm.registerImagePair(self.register_moving_image, self.register_fixed_image,
                                                                               noisy_transformAsVecAffine, True)
                    (Hrgb_estimated_Perspective, mask) = self.algorithm.registerImagePair(self.register_moving_image, self.register_fixed_image,
                                                                               noisy_transformAsVecPerspective, False)                                             
                elif (self.ALGORITHM == Algorithm.MUTUAL_INFORMATION_OCL):
                ...
                for meth in self.reg_methods:
                    #code
                    if (meth == 'withoutMI'):
                        Hrgb_estimated_Perspective_WithoutMI = np.reshape(np.append(noisy_transformAsVecPerspective[0], [1.0], axis=0), (3, 3))
                        Hrgb_estimated = Hrgb_estimated_Perspective_WithoutMI
                        #Hrgb_estimated = np.linalg.inv(noisy_transformAsMatPerspective)
                        #Hrgb_estimated = Hrgb_estimated / Hrgb_estimated[2, 2]
                        Hrgb_bbox_truth_inv = np.linalg.inv(Hrgb_bbox_truth_perspective)
                        Hrgb_bbox_truth_inv = Hrgb_bbox_truth_inv / Hrgb_bbox_truth_inv[2, 2]
                        if (Hrgb_estimated_Perspective_WithoutMI is not None):
                            Herror_withoutMI = Hrgb_estimated - Hrgb_bbox_truth_inv                        
                        else:
                            return
                    if (meth == 'affine'):
                        Hrgb_estimated = Hrgb_estimated_Affine
                        Hrgb_bbox_truth_inv = np.linalg.inv(Hrgb_bbox_truth_affine)
                        Hrgb_bbox_truth_inv = Hrgb_bbox_truth_inv / Hrgb_bbox_truth_inv[2, 2]
                        if (Hrgb_estimated is not None):
                            Herror_affine = Hrgb_estimated - Hrgb_bbox_truth_inv                        
                        else:
                            return
                    if (meth == 'perspective'):
                        Hrgb_estimated = Hrgb_estimated_Perspective
                        Hrgb_bbox_truth_inv = np.linalg.inv(Hrgb_bbox_truth_perspective)
                        Hrgb_bbox_truth_inv = Hrgb_bbox_truth_inv / Hrgb_bbox_truth_inv[2, 2]
                        if (Hrgb_estimated is not None):
                            Herror_perspective = Hrgb_estimated - Hrgb_bbox_truth_inv                           
                        else:
                            return
                    ...
                    if (meth == 'withoutMI'):
                        invPoseTransform_est_withoutMI = invPoseTransform_est
                        yaw_pitch_roll_xyz_err_est_withoutMI = ROSImageRegistrationNode.computePoseError(cameraPose, invPoseTransform_est_withoutMI)
                    if (meth == 'affine'):
                        invPoseTransform_est_affine = invPoseTransform_est
                        yaw_pitch_roll_xyz_err_est_affine = ROSImageRegistrationNode.computePoseError(cameraPose, invPoseTransform_est_affine)
                    if (meth == 'perspective'):
                        invPoseTransform_est_perspective = invPoseTransform_est
                        yaw_pitch_roll_xyz_err_est_perspective = ROSImageRegistrationNode.computePoseError(cameraPose, invPoseTransform_est_perspective)
                ...
                if (self.LOGGING == True):
                    ...
                    self.dumpNumpyDataToLog(invPoseTransform_est_withoutMI)
                    self.dumpNumpyDataToLog(invPoseTransform_est_affine)
                    self.dumpNumpyDataToLog(invPoseTransform_est_perspective)
                    self.dumpAsStringToLog(self.frame_id)
                    self.dumpNumpyDataToLog(yaw_pitch_roll_xyz_err_est_withoutMI)
                    self.dumpNumpyDataToLog(yaw_pitch_roll_xyz_err_est_affine)
                    self.dumpNumpyDataToLog(yaw_pitch_roll_xyz_err_est_perspective)
                    self.dumpNumpyDataToLog(Herror_withoutMI)
                    self.dumpNumpyDataToLog(Herror_affine)
                    self.dumpNumpyDataToLog(Herror_perspective)
                ...
                if (self.IMAGE_DATASET_GT == True):
                    ...
                    self.dumpAsStringToLog(self.frame_id)
                    self.dumpNumpyDataToLog(cameraPose)
                    self.dumpNumpyDataToLog(Hrgb_bbox_truth_inv)
                    #self.dumpNumpyDataToLog(pts_transformed_homogeneous_estimated)
                    self.dumpNumpyDataToLog(ref_poly)
                    self.logfileNewline()
                if (self.DEBUG == True):
                ...
                if (self.ROS_VISUALIZE_SENSOR_VIEW_FRUSTUM == True and uv_target_image is not None):
                    ...
                    if (self.IMAGE_DATASET == True):
                        ...
                        cv2.imwrite('/home/mhossai9/temp/dataset/matched/registration_result_image_'+str(self.frame_id)+'.png', img)
